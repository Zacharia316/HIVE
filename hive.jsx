...
function ProfilePage({ userId, isMe, onBack, onEdit, profile, myId, onSignOut }) {
  ...
  const toggleFollow = async () => {
    ...
    try {
      if (iFollow) {
        await sb.from("follows").delete().eq("follower_id", myId).eq("followee_id", userId);
      } else {
        await sb.from("follows").insert({ follower_id: myId, followee_id: userId });
      }
      await loadFollowState();
    } finally {
      setFollowBusy(false);
        ...
        {editing ? (
          ...
          <div style={{ display: "flex", flexDirection: "column", alignItems: "center" }}>
            <div style={{ position: "relative" }}>
              ...
              {isAdmin && (
                <button
                  ...
                  style={{ position: "absolute", bottom: -2, right: -2, background: "rgba(242,169,59,0.15)", border: "1px solid rgba(242,169,59,0.35)", borderRadius: "50%", width: 24, height: 24, display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer" }}
                >
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 6, marginTop: 12 }}>
              ...
              {isAdmin && (
                <Pencil size={12} color="#6B6F78" style={{ cursor: "pointer" }} onClick={() => setEditing(true)} />
              )}
            </div>
...
function ChannelsPage({ onOpen, myId, onCreate }) {
  ...
  const joinChannel = async () => {
    ...
    try {
      ...
      let { data: c } = await sb.from("channels").select("*").eq("handle", key).maybeSingle();
      if (!c) {
        const { data: c2 } = await sb.from("channels").select("*").eq("id", key).maybeSingle();
        c = c2;
      }
      if (!c) {
        setJoinErr("Couldn't find a channel with that link.");
        return;
      }
        ...
        {editing ? (
          ...
          <div style={{ display: "flex", flexDirection: "column", alignItems: "center" }}>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 4, marginTop: 12 }}>
              ...
              {isAdmin && <Pencil size={12} color="#6B6F78" style={{ cursor: "pointer" }} onClick={() => setEditing(true)} />}
            </div>
