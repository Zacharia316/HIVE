...
function ChatList({ currentUser, profile, onOpen }) {
  ...
  const loadConvos = useCallback(async () => {
    ...
    const seen = {};
    data.forEach(msg => {
      if (!seen[msg.conversation_id]) seen[msg.conversation_id] = msg;
    });

    const otherIds = Object.values(seen).map(msg =>
    ...
    const result = Object.values(seen).map(msg => {
      const otherId = msg.sender_id === currentUser.id ? msg.receiver_id : msg.sender_id;
      return { id: msg.conversation_id, other: profileMap[otherId], lastMsg: msg };
    });
...
// ─── STATUS STORY VIEWER ─────────────────────────────────────────────────────
function StoryViewer({ userStories, allUserStories, onClose, onNext, onPrev }) {
    ...
    <div style={{ position:"fixed", inset:0, background:"#000", zIndex:100, display:"flex", flexDirection:"column", maxWidth:430, margin:"0 auto" }}>
      ...
      <div style={{ position:"relative", zIndex:2, marginTop:"auto", padding:"0 16px 28px", display:"flex", gap:10, alignItems:"center" }}>
        <div style={{ flex:1, background:"rgba(255,255,255,0.12)", borderRadius:24, padding:"11px 16px", backdropFilter:"blur(10px)", border:"1px solid rgba(255,255,255,0.15)" }}>
          <input placeholder={`Reply to @${userStories.username}...`} style={{ background:"none", border:"none", outline:"none", color:"#fff", fontFamily:"inherit", fontSize:14, width:"100%" }} />
        </div>
        <button style={{ background:COLORS.purple, border:"none", borderRadius:"50%", width:42, height:42, display:"flex", alignItems:"center", justifyContent:"center", flexShrink:0, cursor:"pointer", boxShadow:`0 0 20px ${COLORS.purple}66` }}>
          <Icons.Send />
        </button>
      </div>
    </div>
...
function ChannelsTab({ currentUser }) {
  ...
  const loadChannels = async () => {
    const { data } = await sb.from("channels").select("*, channel_admins(user_id), channel_followers(user_id)").order("created_at", { ascending: false });
    setChannels(data || []);
  ...
  const createChannel = async () => {
    ...
    setCreating(true);
    const { data: ch } = await sb.from("channels").insert({ name: chName.trim(), description: chDesc.trim(), created_by: currentUser.id }).select().single();
    if (ch) {
      await sb.from("channel_admins").insert({ channel_id: ch.id, user_id: currentUser.id });
      await sb.from("channel_followers").insert({ channel_id: ch.id, user_id: currentUser.id });
    }
    setChName(""); setChDesc(""); setShowCreate(false); setCreating(false);
    loadChannels();
  };

  const toggleFollow = async (chId) => {
    ...
    <div style={{ flex:1, display:"flex", flexDirection:"column", overflow:"hidden" }}>
      ...
      {showCreate && (
        <div style={{ margin:"0 16px 10px", ...neu(), padding:16, borderRadius:16, flexShrink:0 }}>
          ...
          <input value={chDesc} onChange={e=>setChDesc(e.target.value)} placeholder="Description (optional)" style={{ width:"100%", background:COLORS.bg, border:`1px solid ${COLORS.border}`, borderRadius:10, padding:"10px 12px", color:COLORS.text, fontFamily:"inherit", fontSize:14, outline:"none", boxSizing:"border-box", marginBottom:12 }} />
