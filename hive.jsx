...
function ChannelsTab({ currentUser }) {
  ...
  const createChannel = async () => {
    ...
    const { data: ch } = await sb.from("channels").insert({ name: chName.trim(), handle, owner_id: currentUser.id, allow_responses: true }).select().single();
    if (ch) {
      await sb.from("channel_followers").insert({ channel_id: ch.id, user_id: currentUser.id });
    }
    setChName(""); setChHandle(""); setShowCreate(false); setCreating(false);
    loadChannels();
  };
