...
function ProfilePage({ userId, isMe, onBack, onEdit, profile, myId, onSignOut }) {
  ...
  const loadFollowState = async () => {
    ...
    setKnown(!!f);
    const { data: mine } = await sb.from("follows").select("id").eq("follower_id", myId).eq("followee_id", userId).maybeSingle();
    setIFollow(!!mine);
  };
  ...
  const toggleFollow = async () => {
    ...
    try {
      if (iFollow) {
        const { error } = await sb.from("follows").delete().eq("follower_id", myId).eq("followee_id", userId);
        if (error) { console.error("Unfollow failed:", error); alert("Unfollow failed: " + error.message); return; }
      } else {
        const { error } = await sb.from("follows").insert({ follower_id: myId, followee_id: userId });
