...
function ProfilePage({ userId, isMe, onBack, onEdit, profile, myId }) {
...
function ChatThread({ chatId, partner, myId, onBack, onOpenProfile, onForward, allPartners }) {
  ...
  const endRecord = async () => {
    ...
    setRecording(false);
    const secs = Math.max(1, Math.round((Date.now() - recordStart.current) / 1000));
    await sb.from("messages").insert({ sender_id: myId, receiver_id: chatId, type: "voice", duration: secs, status: "sent" });
  };
                ...
                >
                  ...
                  {msg.type === "image" ? (
                    ...
                    <div style={{ display: "flex", alignItems: "center", gap: 8, minWidth: 140 }}>
                      <Mic size={14} />
                      <div style={{ flex: 1, height: 3, background: mine ? "rgba(26,19,5,0.3)" : "rgba(255,255,255,0.15)", borderRadius: 2 }} />
                      <span style={{ fontSize: 11 }}>0:{String(msg.duration).padStart(2, "0")}</span>
                    </div>
                  ) : isEditing ? (
                    <input
                      autoFocus
...
function ChannelDetail({ channel, myId, onBack, isAdmin }) {
  ...
  const post = async () => {
    if (!draft.trim()) return;
    setDraft("");
    await sb.from("channel_posts").insert({ channel_id: channel.id, content: draft, author_id: myId });
  };
  ...
  if (!profile) {
    ...
    content = (
      <GroupChatThread
        ...
        isAdmin={openGroup.created_by === myId}
      />
    );
  } else if (openChannel) {
    content = <ChannelDetail channel={openChannel} myId={myId} onBack={() => setOpenChannel(null)} isAdmin={openChannel.created_by === myId} />;
  } else if (tab === "chats") {
    content = (
      <ChatsList
        myId={myId}
    ...
    content = <ProfilePage userId={myId} isMe onBack={null} onEdit={() => setEditingProfile(true)} profile={profile} myId={myId} />;
  }
