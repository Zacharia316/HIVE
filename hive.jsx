...
@media (min-width: 460px) and (min-height: 900px) {
  .hive-shell { border-radius: 34px; border: 1px solid rgba(255,255,255,0.06); box-shadow: 0 30px 60px rgba(0,0,0,0.5), inset 0 0 0 1px rgba(255,255,255,0.02); }
}
`;
...
function ProfilePage({ userId, isMe, onBack, onEdit, profile, myId, onSignOut }) {
          ...
          {!isMe && (
            <button
              ...
              style={{
                ...
                padding: "8px 22px",
                borderRadius: 20,
                border: iFollow ? "1px solid rgba(255,255,255,0.15)" : "1px solid rgba(242,169,59,0.4)",
                background: iFollow ? "rgba(255,255,255,0.05)" : "linear-gradient(135deg, #F2A93B, #D98F28)",
                color: iFollow ? "#F1EDE4" : "#1A1305",
                fontSize: 13,
                fontWeight: 600,
                cursor: "pointer",
              }}
          ...
          {isMe && (
            <>
              <div
                ...
                style={{ width: "100%", marginTop: 16, display: "flex", alignItems: "center", justifyContent: "space-between", background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 14, padding: "12px 16px", cursor: "pointer" }}
              >
                <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                  <Moon size={15} color="#8B8F98" />
                  <span style={{ fontSize: 13.5, color: "#F1EDE4" }}>Appearance</span>
                </div>
                <span style={{ fontSize: 12, color: "#6B6F78" }}>Dark · tap for Light</span>
              </div>
              <button
                onClick={onSignOut}
                style={{
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", height: "100%", background: LS.bg }}>
      <div style={{ height: 120, background: LS.accentGradient, position: "relative", flexShrink: 0 }}>
      ...
      <div style={{ flex: 1, overflowY: "auto", padding: "0 20px 24px", display: "flex", flexDirection: "column", alignItems: "center", textAlign: "center" }}>
        <div style={{ marginTop: -46, borderRadius: "50%", padding: 5, background: LS.bg, boxShadow: "0 4px 14px rgba(76,63,180,0.25)" }}>
          <HexAvatar emoji={p?.avatar_url || p?.avatar_emoji} size={84} ring />
        </div>
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", height: "100%" }}>
      ...
      <div style={{ flex: 1, overflowY: "auto", padding: "6px 10px" }}>
        ...
        <div style={{ margin: "16px 10px", padding: "12px 14px", border: "1px dashed rgba(255,255,255,0.15)", borderRadius: 14, display: "flex", flexDirection: "column", gap: 8 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            <Link2 size={15} color="#8B8F98" />
            <input
              value={joinLink}
              onChange={(e) => setJoinLink(e.target.value)}
              onKeyDown={(e) => e.key === "Enter" && joinGroup()}
              placeholder="Paste invite link to join a group"
              style={{ flex: 1, background: "none", border: "none", outline: "none", color: "#F1EDE4", fontSize: 13, fontFamily: "'Inter', sans-serif" }}
            />
            <button
              onClick={joinGroup}
              disabled={!joinLink.trim() || joining}
              style={{ background: "none", border: "none", color: joinLink.trim() ? "#F2A93B" : "#6B6F78", fontSize: 12.5, fontWeight: 600, cursor: joinLink.trim() ? "pointer" : "default" }}
            >
              {joining ? "…" : "Join"}
            </button>
          </div>
        ...
        {editing ? (
          <div style={{ display: "flex", flexDirection: "column", alignItems: "center" }}>
            ...
            <div style={{ display: "flex", gap: 10, marginTop: 14, width: "100%" }}>
              <button onClick={() => setEditing(false)} style={{ flex: 1, padding: "10px", borderRadius: 12, border: "1px solid rgba(255,255,255,0.12)", background: "none", color: "#8B8F98", fontSize: 13, cursor: "pointer" }}>
                Cancel
              </button>
              <button onClick={saveGroupInfo} disabled={saving} style={{ flex: 1, padding: "10px", borderRadius: 12, border: "none", background: "linear-gradient(135deg, #F2A93B, #D98F28)", color: "#1A1305", fontWeight: 600, fontSize: 13, cursor: "pointer" }}>
                {saving ? "Saving…" : "Save"}
              </button>
            </div>
          </div>
          ...
          <div style={{ display: "flex", flexDirection: "column", alignItems: "center" }}>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 6, marginTop: 12 }}>
              <div style={{ fontFamily: "'Space Grotesk', sans-serif", fontWeight: 700, fontSize: 18, color: "#F1EDE4" }}>{group.name}</div>
              {isAdmin && (
                <button
                  onClick={() => setEditing(true)}
                  style={{ background: "rgba(242,169,59,0.12)", border: "1px solid rgba(242,169,59,0.3)", borderRadius: "50%", width: 22, height: 22, display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", flexShrink: 0 }}
                >
                  <Pencil size={11} color="#F2A93B" />
                </button>
              )}
            </div>
        ...
        {editing ? (
          <div style={{ display: "flex", flexDirection: "column", alignItems: "center" }}>
            ...
            <div style={{ display: "flex", gap: 10, marginTop: 14, width: "100%" }}>
              ...
              <button onClick={saveChannelInfo} disabled={saving} style={{ flex: 1, padding: "10px", borderRadius: 12, border: "none", background: "linear-gradient(135deg, #F2A93B, #D98F28)", color: "#1A1305", fontWeight: 600, fontSize: 13, cursor: "pointer" }}>
                {saving ? "Saving…" : "Save"}
              </button>
            </div>
          </div>
...
export default function Hive() {
  ...
  return (
    ...
    <div style={{ width: "100%", height: "100dvh", background: "#0F1013", display: "flex", alignItems: "center", justifyContent: "center", fontFamily: "'Inter', sans-serif", position: "fixed", inset: 0, overflow: "hidden" }}>
      <style>{fontStyle}</style>
      <div
      ...
      >
        ...
        {showNav && (
          <div style={{ flexShrink: 0, display: "flex", justifyContent: "space-around", alignItems: "center", padding: "10px 6px 14px", borderTop: "1px solid rgba(255,255,255,0.06)", background: "rgba(15,16,19,0.9)", backdropFilter: "blur(10px)" }}>
            {TABS.map((t) => {
              const Icon = t.icon;
